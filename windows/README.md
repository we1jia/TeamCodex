...
3. **托盘左右键分流与原生悬浮控制卡片**：
   - **鼠标左键单击/双击**：在任务栏右下角极速唤起专属深色悬浮控制面板（Form 弹窗），支持实时双色圆点状态自检（Codex 挂载/中枢/房间）、一键启动挂载、口令复制、切换中枢、重启注入与检查更新，点击外部或按 `Esc` 自动收起；
   - **鼠标右键单击**：弹出纯净原版上下文菜单，点击【退出】触发操作系统内核级进程终止，0 毫秒秒退无残留；
   - **全局单实例互斥锁（Mutex）**：引入 `Global\TeamCodexTrayMutex`，启动时自动检测并清理旧残留，彻底杜绝多个托盘图标重复堆叠。
4. **彻底告别黑框（静默后台守护）**：
   - 提供 `启动TeamCodex(无黑框静默).vbs`，通过 Windows 宿主进程静默拉起，日常使用没有常驻黑色 CMD 窗口打扰。
5. **一键完全退出与清理工具**：
   - 提供 `退出TeamCodex.cmd`，遇到任何环境卡死或端口残留时，双击即可一键强杀所有守护进程并刷新任务栏托盘缓存。
6. **虚拟机隔离测试模式**：
...
├── 退出TeamCodex.cmd           # [应急工具] 一键强杀后台进程并刷新托盘
