(() => {
  ...
  const labels = { kanban: '看板', knowledge: '知识库', materials: '素材库' };
  ...
  window.TeamWorkspace = { mount(container, adapter) {
    ...
    const renderEnrollment = () => {
      body.innerHTML = `<div class="tw-enroll"><h1>${escape(labels[page])}</h1><p class="tw-muted">在当前空间启用协作身份。成员凭证保存在此设备，私人资料不会共享给其他成员。</p><form class="tw-form" data-enroll>${field('显示名称', input('name', adapter.getScope().nickname || '团队成员', 'text', 'required maxlength="200"'))}${field('成员类型', `<select name="kind">${options({ human: '团队成员', ai: 'Agent' })}</select>`)}<p class="tw-error" role="alert"></p><button class="tw-btn tw-primary" type="submit">启用工作区</button>${button('chat', '返回对话')}</form><p class="tw-muted">同一人更换设备需安全迁移成员凭证；昵称不是登录凭据。</p></div>`;
